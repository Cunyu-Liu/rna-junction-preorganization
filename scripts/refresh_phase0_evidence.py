...
def main() -> int:
    ...
    append_unique({"source_id": "deenalattha_2026_dms", "kind": "public_processing_source_readme_snapshot", **rel_artifact(args.artifact_root, "phase0/source_metadata/dms_official_readme_ed75e36_20260802.md", status="PUBLIC_PROCESSING_README_SNAPSHOT_PINNED", source_commit="ed75e36bb36dc2f32c866c436c437d88a4743cf8", documented_processed_payload="data.zip", documented_file_id="66482813", raw_sequence_content_emitted=False, primary_labels_admitted=False, scientific_gate_effect="NO_PHASE_0_PASS")})
    ...
    append_unique({"source_id": "deenalattha_2026_dms", "kind": "DMS_reconstruction_feasibility_audit", **rel_artifact(args.artifact_root, "phase0/audits/dms_reconstruction_feasibility_audit_20260802.json", status="BLOCKED_RECONSTRUCTION_INPUTS_MISSING", public_fastq_pair_audits_available=True, source_defined_mutation_histograms_available=False, construct_reference_fasta_available=False, processed_construct_json_available=False, raw_sequence_content_emitted=False, primary_labels_admitted=False, scientific_gate_effect="NO_PHASE_0_PASS")})
    ...
    for source in registry.get("sources", []):
        if source.get("source_id") == "deenalattha_2026_dms":
            ...
            source["processed_dms_payload_route_reprobe"] = "phase0/audits/figshare_route_reprobe_20260801T194600Z.json"
            source["processed_dms_payload_route_reprobe_status"] = "ROUTE_REPROBE_BLOCKED_NO_2XX"
            source["processed_dms_payload_readme_route_reprobe"] = "phase0/audits/figshare_readme_reprobe_20260801T195800Z.json"
    ...
    for path in ("phase0/source_metadata/figshare_route_probe_20260801T192200Z.tsv", "phase0/audits/figshare_dms_processed_payload_route_audit_20260801T192200Z.json"):
        if path not in acceptance["evidence_paths"]:
            acceptance["evidence_paths"].append(path)
    for path in ("phase0/source_metadata/figshare_route_reprobe_20260801T194600Z.tsv", "phase0/audits/figshare_route_reprobe_20260801T194600Z.json"):
        if path not in acceptance["evidence_paths"]:
            acceptance["evidence_paths"].append(path)
    for path in ("phase0/source_metadata/figshare_readme_reprobe_20260801T195800Z.tsv", "phase0/audits/figshare_readme_reprobe_20260801T195800Z.json"):
        if path not in acceptance["evidence_paths"]:
            acceptance["evidence_paths"].append(path)
    ...
    if "phase0/audits/dms_reconstruction_feasibility_audit_20260802.json" not in acceptance["evidence_paths"]:
        acceptance["evidence_paths"].append("phase0/audits/dms_reconstruction_feasibility_audit_20260802.json")
    ...
    if blocker not in blockers:
        blockers.append(blocker)
    blocker = "A low-frequency re-probe of the five recorded official processed-DMS routes again returned HTTP 403 with no 2xx response; the payload remains unavailable through the tested public routes and no access-control bypass is permitted."
    if blocker not in blockers:
        blockers.append(blocker)
    blocker = "The official README's documented User-Agent/Referer request was reproduced for all five recorded Figshare routes and still returned HTTP 403 with no 2xx response; data.zip remains unavailable and no access-control bypass is permitted."
    if blocker not in blockers:
        blockers.append(blocker)
    ...
    if blocker not in blockers:
        blockers.append(blocker)
    blocker = "The raw FASTQ reconstruction prerequisites are incomplete: rna-map and required reference/processed artifacts are absent; FASTQ file-integrity evidence cannot substitute for construct-level DMS hierarchy."
    if blocker not in blockers:
        blockers.append(blocker)
