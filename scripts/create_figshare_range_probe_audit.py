#!/usr/bin/env python3
"""Create an append-only audit for a failed 128 MiB Figshare Range probe."""
...
def parse_headers(path: Path) -> dict[str, object]:
    ...
    return {"status_code": status_code, "content_range": content_range, "content_length": content_length}
...
def main() -> int:
    ...
    if body_path.stat().st_size != 0:
        raise SystemExit("range probe body is non-empty; do not classify as blocked no-payload")
    ...
    payload = {
        "schema_version": "phase0-figshare-range-probe-audit-v1",
        ...
        "observed_content_range": header_data.get("content_range"),
        "observed_content_length_header": header_data.get("content_length"),
        ...
        "next_action": "Do not retry as a payload transfer until an official route returns 2xx with a verified Content-Range and total size.",
    }
