"""Definitive submission horizontal comparison table v4 (r62 frozen method).

v4 adds the r62 treatment: r56b per-context EB mu (Stage 1) + INDEPENDENT
per-scaf x stratum sigma re-scan on the corrected mu (Stage 2).  r62 fixes
r56b's coupling defect that made emitted sigma_m systematically too small
(scaf8 0.530 vs 0.599); under the heavy-tailed measured residuals (kurtosis
~1.1) the optimal Gaussian sigma is larger, so decoupling lowers pooled NLL.

FROZEN submission method: 7-member ensemble (equal-family wg=0.5) + r62
(kappa=1, min_meas=3) = 0.7243.  Absolute NLL chain: nuisance 1.0916 ->
r45 0.7907 -> r51 0.7815 -> r56b 0.7314 -> r62 0.7243.

HONEST CAVEAT (must be in manuscript): r62 vs r56b pooled gain -0.0071 but the
edit-cluster CI [-0.0073, 0.0197] crosses 0 -- the gain is concentrated in the
heavy-tail folds (CUAAG_CUUAG 3.06->2.77, CUCAG_CUGAG 3.11->2.82), while
low-NLL folds slightly worsen.  r62 is an implementation correction (decoupled
sigma scan), not a new modeling lever; the pooled estimand is consistently
lower across all mm3 configs.

Columns (per family): frozen 0.7 / r45 / r51 / r56b / r62.
Output: submission_horizontal_table_v4.json + markdown.
"""
...
def main():
    ...
    for name, preds in models.items():
        ...
        cal_r62c, _ = _calibrate_r62(preds, folds, kappa=1.0, min_meas=3)
        cal_r62[name] = round(_pooled(cal_r62c), 4)
    ...
    out["r62_calibrated_nll"] = cal_r62
    out["r62_rel_gain_pct_vs_nuisance"] = {
        k: round(100.0 * (cal_r62[NUIS] - v) / cal_r62[NUIS], 2)
        for k, v in cal_r62.items() if k != NUIS}

    # Frozen submission method = r62 on 7-member equal-family ensemble
    folds7 = sorted(set(ens7_wg05[r]["fold"] for r in ens7_wg05))
    cal7_r62, _ = _calibrate_r62(ens7_wg05, folds7, kappa=1.0, min_meas=3)
    ...
    out["frozen_method_r62_nll"] = round(_pooled(cal7_r62), 4)
    out["frozen_method_r62_strata"] = _pooled_strata(cal7_r62)
    out["frozen_method_r62_vs_nuisance_r45"] = {
        "rel_gain_pct": round(100.0 * (_pooled(cal_nuis_r45) - _pooled(cal7_r62)) / _pooled(cal_nuis_r45), 2),
        "edit_cluster_CI": _edit_ci(cal7_r62, cal_nuis_r45),
    }
    out["frozen_method_r62_vs_r56b"] = {
        "delta": round(_pooled(cal7_r62) - _pooled(cal7_r56), 4),
        "edit_cluster_CI": _edit_ci(cal7_r62, cal7_r56),
    }
    out["old_frozen_r56b_nll"] = round(_pooled(cal7_r56), 4)

    out["note"] = (
        "DEFINITIVE submission horizontal table v4.  Frozen = 7-member ensemble "
        "(wg=0.5) + r62 (r56b per-context EB mu + decoupled per-scaf x stratum "
        "sigma re-scan, kappa=1, min_meas=3) = 0.7243.  r62 fixes r56b's "
        "coupling defect (sigma_m too small under heavy tails); pooled NLL "
        "consistently lower across all mm3 configs, but edit-cluster CI vs "
        "r56b crosses 0 (gain concentrated in heavy-tail folds) -- report as "
        "an implementation correction with the pooled estimand as primary."
    )
    Path(f"{R}/submission_horizontal_table_v4.json").write_text(
        json.dumps(out, indent=2, sort_keys=True) + "\n")

    lines = ["| model | sigma=0.7 | r45 | r51 | r56b | r62 |",
             "|---|---:|---:|---:|---:|---:|"]
    for name in sorted(frozen, key=lambda k: cal_r62[k]):
        lines.append(f"| {name} | {frozen[name]:.4f} | {cal_r45[name]:.4f} "
                     f"| {cal_r51[name]:.4f} | {cal_r56[name]:.4f} | {cal_r62[name]:.4f} |")
    md = "\n".join(lines)
    Path(f"{R}/submission_horizontal_table_v4.md").write_text(md + "\n")
