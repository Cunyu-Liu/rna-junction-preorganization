"""r71: verify r70 quadrature err10-sigma with edit-cluster CI + mechanism.

r70 found quadrature sigma = sqrt(sigma_scaf^2 + (c*err10)^2) with per-fold
fitted c gives 0.7199 vs r62 0.7243 (-0.0044), the first positive since r62.
Verify:
  1. edit-cluster CI (bootstrapped over 37 edit components) for r70 vs r62.
  2. per-fold fitted c values and stability (leave-one-fold-out).
  3. strata split (measured NLL by scaf).
  4. honest caveat: c fitted on OTHER folds, applied to held-out fold (LOO, no
     leakage); check the fit is not dominated by one fold.
"""
...
def main():
    ...
    def _fit_c_loo():
        """Fit c per fold on OTHER folds (same as r70), return c per fold."""
        ...
        for f in folds:
            ...
            base = np.asarray([scaf_sigma.get(int(p["scaf"]), sm_global or 0.5)
                               for p in fit], dtype=float)
    ...
    cal70 = _apply(c_by_fold)
    nll70 = _pooled(cal70)
    print(f"r62  = 0.7243  (frozen)")
    print(f"r70  = {nll70:.4f}   delta = {nll70-0.7243:+.4f}")

    ci = _edit_ci(cal70, cal62)
    print(f"edit-cluster CI r70 vs r62 = {ci['ci']}  lower_gt_0={ci['ci_lower_gt_0']}  "
          f"leave_one_largest={ci['leave_one_largest']} (largest={ci['largest']})")

    # per-fold c
    print("\nper-fold fitted c:")
    cf = sorted(c_by_fold.items(), key=lambda kv: kv[1])
    for f, c in cf:
        print(f"  {f}: c={c}")
    ...
    def _strata(cal):
        ...
        return out
    print("\nstrata:")
    print("  r62:", {k: round(v, 4) for k, v in _strata(cal62).items()})
    print("  r70:", {k: round(v, 4) for k, v in _strata(cal70).items()})

    # per-scaf measured NLL comparison
    def _per_scaf(cal):
        out = defaultdict(list)
        for rid, p in cal.items():
            if not p["cens"]:
                nll = float(row_nll([p["y"]], [p["cens"]], [p["mu"]], [p["sigma"]])[0])
                out[int(p["scaf"])].append(nll)
        return {k: float(np.mean(v)) for k, v in sorted(out.items())}
    ps62, ps70 = _per_scaf(cal62), _per_scaf(cal70)
    print("\nper-scaf measured NLL (r62 -> r70, delta):")
    for sc in sorted(ps62):
        print(f"  scaf{sc}: {ps62[sc]:.4f} -> {ps70[sc]:.4f} ({ps70[sc]-ps62[sc]:+.4f})")

    out = {
        "r62": 0.7243,
        "r70": round(nll70, 4),
        "delta": round(nll70 - 0.7243, 4),
        "edit_cluster_CI_vs_r62": ci,
        "c_by_fold": {str(k): v for k, v in c_by_fold.items()},
        "strata_r70": {k: round(v, 4) for k, v in _strata(cal70).items()},
        "per_scaf_measured_r70": {str(k): round(v, 4) for k, v in ps70.items()},
        "note": "r70 = r62 mu + quadrature per-row sigma sqrt(sigma_scaf^2+(c*err10)^2); c fitted LOO on other folds.",
    }
    Path("/mnt/cunyuliu/rna_junction_repair_20260811T090000Z/"
         "r71_err10_sigma_verify.json").write_text(json.dumps(out, indent=2) + "\n")
