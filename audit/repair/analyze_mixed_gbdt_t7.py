"""Mixed-family ensemble: 3x multi-seed Gaussian GBDT + 3x t7 MLP.

Consumes materialized preds from r24 (3x t7 MLP seeds), r33 (Gaussian XGBoost
seed23 + nuisance), and r34_gbdt_seeds_full (Gaussian XGBoost seeds s99/s2026)
on the same 37 blocked joint folds, restricted to optimizer+full-coverage
eligible rows (fail-closed).

Headline estimand: pooled junction-macro right-censored Gaussian NLL (sigma=0.7).

Ensembles (mu-average, fixed sigma=0.7):
  - GBDT family: 3x Gaussian seeds {s23, s99, s2026}
...
Note: a Student-t (df=7) objective was implemented for the GBDT family and
smoke-tested (r34_gbdt_seeds_t7_smoke2); it did NOT beat the Gaussian objective
and one seed was unstable, so the robust-likelihood result does not transfer to
Newton boosting.  This file therefore uses only the Gaussian GBDT members.
"""
...
def main():
    ...
    for m in (XGB_S99, XGB_S2026):
        members[m] = _by_rid(rows34, m, elig34)
    for m in MLP:
    ...
    ens_g3 = _ens(members, GBDT)
    ens_m3 = _ens(members, MLP)
    ens_prev4 = _ens(members, [XGB] + MLP)  # previous headline (xgb + 3x t7)
    ens_mixed2 = _ens({"g": ens_g3, "m": ens_m3}, ["g", "m"])

    out["ensemble_nll"] = {
        "gbdt_gauss_3x": round(_pooled(ens_g3), 4),
        "mlp_t7_3x": round(_pooled(ens_m3), 4),
        "xgb_plus_3xt7_4member": round(_pooled(ens_prev4), 4),
        "mixed_gbdt3x_t7_3x_2": round(_pooled(ens_mixed2), 4),
    }
    out["ensemble_rel_gain_pct"] = {
        k: round(100.0 * (out["nuisance_nll"] - v) / out["nuisance_nll"], 2)
        for k, v in out["ensemble_nll"].items()}
    out["edit_cluster_CI_mixed_vs_nuisance"] = _edit_ci(ens_mixed2, nuis)
    out["n_rows"] = len(ens_mixed2)
    out["note"] = ("GBDT family is averaged over 3 independent Gaussian seeds "
                   "first, then mixed with the 3x t7 MLP family by mu-mean; the "
                   "previous headline (xgb + 3x t7 = 0.8599) is the 4-member "
                   "contrast above. Student-t GBDT was smoke-rejected.")
