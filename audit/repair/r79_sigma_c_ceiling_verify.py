"""r79: verify r78 sigma_c ceiling extension - CI, stability, higher ceilings.

r78 found extending sigma_c ceiling from 1.6 to 5.0 lowers pooled NLL 0.7243 ->
0.7199 (censored 0.199 -> 0.1786).  Verify:
  1. edit-cluster CI (r78 ceiling=5.0 vs r62).
  2. even higher ceilings (7.5, 10) - is 5.0 the plateau?
  3. per-fold sigma_c distribution + which folds drive the gain.
  4. mechanism: scaf8 censored rows want huge sigma_c (mu close to CAP, ambiguous).
"""
...
def main():
    ...
    for ceiling in (5.0, 7.5, 10.0):
        ...
        results[str(ceiling)] = {"nll": round(nll, 4),
                                 "strata": {k: round(v, 4) for k, v in _pooled_strata(cal).items()}}
        print(f"ceiling={ceiling}: pooled={nll:.4f} "
              f"strata={ {k: round(v,4) for k,v in _pooled_strata(cal).items()} }")
        if ceiling == 5.0:
            ci = _edit_ci(cal, cal62)
            print(f"  edit CI vs r62 = {ci['ci']} lower_gt_0={ci['ci_lower_gt_0']} "
                  f"leave1={ci['leave_one_largest']} (largest={ci['largest']})")
            # per-fold sigma_c for scaf8 across folds
            sig8 = defaultdict(list)
            for rid, p in cal.items():
                if p["cens"] and int(p["scaf"]) == 8:
                    sig8[p["fold"]].append(p["sigma"])
            print("  scaf8 sigma_c per fold:", {f: round(float(np.median(v)), 2)
                                                for f, v in sorted(sig8.items())})
            sig9 = defaultdict(list)
            for rid, p in cal.items():
                if p["cens"] and int(p["scaf"]) == 9:
                    sig9[p["fold"]].append(p["sigma"])
            print("  scaf9 sigma_c per fold:", {f: round(float(np.median(v)), 2)
                                                for f, v in sorted(sig9.items())})
            # mechanism: scaf8 censored mu vs CAP
            mu8 = [p["mu"] for p in cal.values() if p["cens"] and int(p["scaf"]) == 8]
            print(f"  scaf8 censored mu: min={min(mu8):.2f} med={np.median(mu8):.2f} "
                  f"max={max(mu8):.2f}  (CAP=-7.1)")

    out = {"r62": round(_pooled(cal62), 4), "results": results,
           "note": "extend sigma_c ceiling: 5.0 gives 0.7199; verify plateau/CI"}
    Path("/mnt/cunyuliu/rna_junction_repair_20260811T090000Z/"
         "r79_sigma_c_ceiling_verify.json").write_text(json.dumps(out, indent=2) + "\n")
