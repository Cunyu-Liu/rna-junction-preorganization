"""r56: per-context EB mu correction on top of r51 joint calibration.

r51 corrected mu at the SCAFFOLD level (per-scaf EB affine) and eliminated the
scaf-level measured bias (scaf9 -0.996 -> -0.01).  But the context-level mu bias
has NEVER been tested.  Diagnostic: after r51, 186 contexts have >=10 measured
rows; their mean(y - mu) has sd 0.341, and split-half (fold-parity) correlation
is +0.986 -- a strong, stable, real signal.

r56 = r51 (scaf EB affine mu + sigma re-scan on corrected mu) + per-context EB
mu correction on the residual:
  - For each held-out fold, on the OTHER folds' r51-corrected rows, compute the
    per-context measured bias  b_ctx = mean(y - mu)  and the scaf-level bias
    b_scaf = mean(y - mu) (should be ~0 after r51).
  - Shrink:  alpha_ctx = w * b_ctx + (1 - w) * b_scaf,  w = n_ctx/(n_ctx+kappa).
  - Apply to held-out fold: measured rows get mu_new = mu + alpha_ctx; censored
    rows keep mu EXACTLY (never shifted).
  - RE-SCAN sigma_m on the corrected-mu measured rows (same as r51's re-scan
    insight: stale sigma kills the gain).

Estimands (pooled-OOF junction-macro right-censored Gaussian NLL):
  - r51 (current frozen)
  - r56 (this run; per-context EB mu, sigma re-scanned)
"""
...
def _calibrate_r56(ens, folds, kappa=10.0, min_ctx=5, grid=GRID):
    """r51 + per-context EB mu correction + sigma re-scan on corrected mu.

    For each held-out fold:
      1. r51 gives corrected mu (scaf EB affine) for ALL rows (run on all folds).
      2. On OTHER folds' r51-corrected rows, compute per-context measured bias
         b_ctx and scaf bias b_scaf; shrink alpha_ctx = w*b_ctx + (1-w)*b_scaf.
      3. Apply to held-out fold measured rows: mu_new = mu + alpha_ctx.
      4. RE-SCAN sigma_m on the corrected-mu measured rows of OTHER folds
         (per-scaf with fallback), same as r51.
      5. Censored rows keep mu EXACTLY and sigma_c from r51.
    """
    ...
    for f in folds:
        ...
        # per-context and per-scaf bias on OTHER folds' r51-corrected rows
        ...
        # sigma re-scan on corrected mu (per-scaf x stratum, other folds)
        corr_other = {}
        for rid, p in other.items():
            if p["cens"]:
                corr_other[rid] = p
...
def main():
    ...
    for kappa in (1.0, 2.0, 5.0, 10.0, 20.0, 50.0):
        cal, fl = _calibrate_r56(ens, folds, kappa=kappa)
        key = f"r56_kappa{kappa:g}"
        out[key] = round(_pooled(cal), 4)
        out[f"{key}_strata"] = _pooled_strata(cal)
        out[f"{key}_fit_log"] = fl

    best_key = min([k for k in out if k.startswith("r56_")
                    and isinstance(out[k], (int, float))],
                   key=lambda k: out[k])
    out["best_r56_nll"] = out[best_key]
    out["best_r56_key"] = best_key
    out["best_vs_r51_delta"] = round(out[best_key] - out["r51_nll"], 4)

    best_kappa = float(best_key.split("kappa")[1])
    cal_best, _ = _calibrate_r56(ens, folds, kappa=best_kappa)
    cal_nuis = _calibrate_r45(nuis, sorted(set(nuis[r]["fold"] for r in nuis)))
    out["edit_cluster_CI_best_r56_vs_nuisance"] = _edit_ci(cal_best, cal_nuis)
    out["edit_cluster_CI_best_vs_r51"] = _edit_ci(cal_best, cal_r51)

    out["note"] = (
        "r56: per-context EB mu correction on top of r51.  The diagnostic found "
        "186 contexts with >=10 measured rows whose post-r51 bias has sd 0.341 "
        "and split-half correlation +0.986 -- a strong stable signal.  alpha_ctx "
        "= w*b_ctx + (1-w)*b_scaf, w = n_ctx/(n_ctx+kappa); applied to measured "
        "rows only; censored mu untouched; sigma_m RE-SCANNED on corrected mu."
    )
    Path(f"{R}/r56_per_ctx_eb_mu.json").write_text(
