"""k-mer-representation latent-operator adapter (representation shootout).

Identical to corrected_v1_31 EXCEPT the sequence map: the 63-D position/
composition features are replaced by normalized k-mer composition (k=3).
The latent-operator head, GH integration, scaffold intercept/slope, ridge,
bounds, optimizer budget and strict projected-gradient gate are unchanged.

Model:
    q_j ~ N(f_theta(seq), sigma_q^2),  f = x_j @ theta,  x_j = k-mer features
...
vs the no-sequence model is attributable to the k-mer representation.
"""
...
from audit.benchmark.kmer_features import build_raw_by_jid, fit_scaler, transform
...
def _panel_and_x(train_rows, test_rows):
    ...
    by_jid = build_raw_by_jid(train_rows + test_rows, k=K)
    mean, sd = fit_scaler(tr_jids, by_jid, k=K)
    X_tr = transform(tr_jids, by_jid, mean, sd, k=K)
    X_te = transform(te_jids, by_jid, mean, sd, k=K)
...
def make_kmer_latent_adapter():
    """Return (fit, predict) for the k-mer-representation latent operator."""
    def fit(train_rows):
        ...
        by_jid = build_raw_by_jid(train_rows, k=K)
        return {"kind": "kmer_latent_operator", "theta": theta, "a": a, "b": b,
    ...
    def predict(model, test_rows):
        mean, sd = fit_scaler(model["tr_jids"], model["by_jid"], k=K)
        te_jids = sorted({str(r["jid"]) for r in test_rows})
        by_jid = build_raw_by_jid(test_rows, k=K)
        X_te = transform(te_jids, by_jid, mean, sd, k=K)
...
KMER_LATENT_OPERATOR = {
    "kmer_latent_operator": make_kmer_latent_adapter(),
}