"""Unit tests for the ViennaRNA-extended plain-linear hybrid."""
import numpy as np

from audit.models.vienna_extended_linear_hybrid import (
    VIENNA_EXTENDED_LINEAR_HYBRID, make_vienna_extended_linear_hybrid)
from audit.benchmark.vienna_extended_features import raw_features
...
def test_model_registered():
    assert "vienna_extended_linear_hybrid" in VIENNA_EXTENDED_LINEAR_HYBRID
    fit, predict = VIENNA_EXTENDED_LINEAR_HYBRID["vienna_extended_linear_hybrid"]
    assert callable(fit) and callable(predict)


def test_feature_dimension_is_21():
    assert raw_features("CUAG_CUAAG").shape == (21,)


def test_fit_predict_shapes_and_finiteness():
    fit, predict = make_vienna_extended_linear_hybrid()
    rows = _rows()
    tr, te = rows[:18], rows[18:]
    model = fit(tr)
    assert model["kind"] == "vienna_extended_linear_hybrid"
...
def test_sequence_block_changes_predictions():
    fit_fn, predict_fn = make_vienna_extended_linear_hybrid()
    rows = _rows()
...
def test_train_only_scaling_no_leakage():
    ...
    fit_fn, _ = make_vienna_extended_linear_hybrid()
    rows = _rows()
    tr = rows[:18]
    model = fit_fn(tr)
...
if __name__ == "__main__":
    import sys
    sys.path.insert(0, ".")
    tests = [test_model_registered, test_feature_dimension_is_21,
             test_fit_predict_shapes_and_finiteness, test_unseen_scaffold_abstains,
             test_sequence_block_changes_predictions, test_train_only_scaling_no_leakage]
    failed = 0
    for t in tests:
        try:
            t()
    ...
    print("vienna_extended_linear_hybrid tests", "PASS" if failed == 0 else f"{failed} FAILURES")